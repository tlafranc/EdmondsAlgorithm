...
int main() {
    ...
    for (int t = 0; t < T; t++) {
        ...
        // 2. There is already an edge (u,v) with weight w' and w >= w'
        for (int i = 0; i < m; i++) {
            int u, v, w;
            cin >> u;
            cin >> v;
...
int findACycle(int vertex, unordered_map<int, Edge*>& vertexToMinEdge, unordered_set<int>& toRoot, unordered_set<int>& currentPath, unordered_set<Edge*>& cycleEdges, unordered_set<int>& cycleNodes) {        
    ...
    if (parentVertex == 0) { // root vertex
        return false;
    } 

    if (toRoot.find(parentVertex) != toRoot.end()) { // Parent vertex leads to root
        toRoot.insert(vertex);

        return false;
    } else if (currentPath.find(parentVertex) == currentPath.end()) { // Haven't seen parent vertex before so recurse
        int foundCycle = findACycle(parentVertex, vertexToMinEdge, toRoot, currentPath, cycleEdges, cycleNodes);
        if (foundCycle && foundCycle != parentVertex) { // Second check is to see if vertex does not belong in foundCycle
    ...
    } else { // Have seen parent vertex in current path before so this is a cycle
        cycleEdges.insert(edge);
        cycleNodes.insert(vertex);
